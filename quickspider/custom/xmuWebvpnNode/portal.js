...
var getPortalGroupSidebarHtml = function (data) {
    ...
    for (var i in groups) {
        ...
        htmlText += '<div class="sidebar-tree__item"><a href="#group-'+group.id+'">'+group.group_name+'</a></div>'
    }
...
var loadBody = function() {
    $.ajax({
        ...
        success: function (res) {
            ...
            $("#vpn-content").html($("#vpn-content").html() + getHistory() + getCollect() + sidebarHtml + contentHtml)
...
function addHtmlToHistory(data) {
    ...
    var sidebarHtml = '<div id="historySideBar" class="sidebar-tree__item"><a href="#history">最近访问</a></div>';
    if (data.length > 0) {
        for (var i in data) {
        ...
        $("#history").sortable({
            ...
            update: function (event, ui) {
                ...
                var newRecentData = newOrder.map(function(v, i) { return recentData[v]})
...
function initCollectionCss() {
    collectionUrls = collectionData.map(function(item) {return item.url})
...
function addHtmlToCollect(data) {
    ...
    var sidebarHtml = '<div id="collectSideBar" class="sidebar-tree__item"><a href="#collect">我的收藏</a></div>'
    if (data.length > 0) {
        ...
        $("#collect").sortable({
            ...
            update: function (event, ui) {
                ...
                var newCollectData = newOrder.map(function(v, i) { return collectionData[v]})
